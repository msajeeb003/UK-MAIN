...
| Method | Path | Purpose |
|---|---|---|
| `GET` / `PUT` | `/config/insurers` | `insurers: [{id, name, legal_names, debt_collection: included\|outsourced, debt_collection_label?, active}]` plus `version`, `updated_by`, `updated_at`, `source`. `name` is the client-facing display name used for column headings and the slides; `legal_names` are for identification only. `debt_collection_label` is optional wording shown for that insurer's Debt collection cell in place of Included / Outsourced (Cartan: "Inclusive collections"); the review screen offers it as a per-column choice. Ids are slugs; names must be unique across canonical and legal names; at least one insurer active. An insurer that still has terminology cannot be removed. |
| `GET` / `PUT` | `/config/terminology` | `fields: {standard field → [terms]}` for wording any insurer uses and `insurers: {insurer id → {standard field → [terms]}}` for one insurer's wording; every field key must be one of the 16 standard terms (`standard_fields` in the response), insurer keys must be standing-list ids. `rules: {insurer id \| "*" → {standard field → [{match, render}]}}` normalise an insurer's sentence to the presentation's short form after verification (Nexus: "Open credit not exceeding 60 days from end of month …" → "60 days end of month"): `match` is a case-insensitive regex with named groups, `render` a template using those groups, so the figure is always the document's own. |
| `GET` | `/presentation/wording` | The fixed presentation wording (`backend/config/presentation.json`: brand, About Us, Feedback of Terms, the five-paragraph Important-information block, demands & needs, the recommendation sentence with `{name}`, our status, contact). Configuration, re-read on change; never editable out of a generated deck. |
